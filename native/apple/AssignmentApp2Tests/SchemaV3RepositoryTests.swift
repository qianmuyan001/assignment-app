...
private func v3WithSQLite<T>(
    ...
    guard sqlite3_open_v2(url.path, &database, flags, nil) == SQLITE_OK,
          let database else {
        if let database { closeTestSQLiteConnection(database) }
...
struct SchemaV3RepositoryTests {
    ...
    func sharedV2FixtureMigration() throws {
        try withTemporarySQLiteDatabase(fileName: "fixture-v2.db") { temporary in
    ...
    func failureRecoveryFingerprint() throws {
        try withTemporarySQLiteDatabase(fileName: "recovery-v2.db") { temporary in
            try createV2Fixture(at: temporary.databaseURL)
    ...
    func rollbackExternalWriteIsPreserved() throws {
        try withTemporarySQLiteDatabase(fileName: "rollback-external-v2.db") { temporary in
            try createV2Fixture(at: temporary.databaseURL)
            ...
            do {
                _ = try MigrationCoordinator.prepareDatabase(
                    ...
                    postRollbackFailureInjector: {
                        ...
                        defer { closeTestSQLiteConnection(external) }
    ...
    func attachmentPayloadLifecycle() throws {
        try withTemporarySQLiteDatabase(fileName: "attachment-files.db") { temporary in
    ...
    func concurrentMigrationInitialization() throws {
        try withTemporarySQLiteDatabase(fileName: "concurrent-v2.db") { temporary in
            try createV2Fixture(at: temporary.databaseURL)
    ...
    func postCommitFailurePreservesCommittedCandidate() throws {
        try withTemporarySQLiteDatabase(fileName: "post-commit-v2.db") { temporary in
            try createV2Fixture(at: temporary.databaseURL)

            let oldConnection = try SQLiteSupport.open(temporary.databaseURL)
            defer { closeTestSQLiteConnection(oldConnection) }
    ...
    func postCommitExternalWriteIsPreserved() throws {
        try withTemporarySQLiteDatabase(fileName: "post-commit-external-v2.db") { temporary in
            try createV2Fixture(at: temporary.databaseURL)
    ...
    func extensionFingerprintAndMigration() throws {
        try withTemporarySQLiteDatabase(fileName: "extensions-v2.db") { temporary in
            try createV2Fixture(at: temporary.databaseURL)
    ...
    func strictSchemaMetadataAndCompletionValidation() throws {
        try withTemporarySQLiteDatabase(fileName: "invalid-completed.db") { timestampDatabase in
            do {
        ...
        try withTemporarySQLiteDatabase(fileName: "invalid-index.db") { indexDatabase in
    ...
    func strictOrganizationRowValidation() throws {
        try withTemporarySQLiteDatabase(fileName: "invalid-parent-state.db") { stateDatabase in
        ...
        try withTemporarySQLiteDatabase(fileName: "invalid-rrule.db") { reminderDatabase in
        ...
        try withTemporarySQLiteDatabase(fileName: "invalid-blob.db") { attachmentDatabase in
        ...
        try withTemporarySQLiteDatabase(fileName: "invalid-course-snapshot.db") { relationshipDatabase in
        ...
        try withTemporarySQLiteDatabase(fileName: "invalid-scalar.db") { scalarDatabase in
    ...
    func subtaskProgressMatrix() throws {
        try withTemporarySQLiteDatabase(fileName: "subtask-matrix.db") { temporary in
            let tasks = try SQLiteAssignmentRepository(databaseURL: temporary.databaseURL)
            let organization = try SQLiteOrganizationRepository(databaseURL: temporary.databaseURL)
    ...
    func restoreAndOrganizationInvariants() throws {
        try withTemporarySQLiteDatabase(fileName: "restore-contract.db") { temporary in
            let tasks = try SQLiteAssignmentRepository(databaseURL: temporary.databaseURL)
            let organization = try SQLiteOrganizationRepository(databaseURL: temporary.databaseURL)
    ...
    func dueTimezoneAndRepeatRuleValidation() throws {
        try withTemporarySQLiteDatabase(fileName: "due-rules-v2.db") { temporary in
            try createV2Fixture(at: temporary.databaseURL)
            try v3WithSQLite(at: temporary.databaseURL) {
                try SQLiteSupport.execute(
