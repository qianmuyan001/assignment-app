...
private struct LearningSceneFixture {
    let database: TemporarySQLiteDatabase
...
private func withLearningSceneFixture(
    fileName: String = "assignments.db",
    courseName: String = "Physics 物理",
    _ body: (LearningSceneFixture) throws -> Void
) throws {
    try withTemporarySQLiteDatabase(fileName: fileName) { database in
...
struct LearningSceneRepositoryTests {
    ...
    func v3ToV4PreservesExtensions() throws {
        try withTemporarySQLiteDatabase(fileName: "v3-extension.db") { temporary in
            try makeV3Database(at: temporary.databaseURL)
    ...
    func v3ToV4FailureRollsBack() throws {
        try withTemporarySQLiteDatabase(fileName: "v3-failure.db") { temporary in
            try makeV3Database(at: temporary.databaseURL)
            let remindersBefore = try learningRows(at: temporary.databaseURL, learningReminderSnapshotSQL)
    ...
    func partialV4IsRejected() throws {
        try withTemporarySQLiteDatabase(fileName: "partial-v4.db") { temporary in
            try makeV3Database(at: temporary.databaseURL)
            try learningWithSQLite(at: temporary.databaseURL) { database in
                try SQLiteSupport.configure(database)
                try SQLiteSupport.execute(
    ...
    func v4OpensWithoutRepeatMigration() throws {
        try withTemporarySQLiteDatabase(fileName: "already-v4.db") { temporary in
            try makeV3Database(at: temporary.databaseURL)
    ...
    func freshDatabaseIsV4() throws {
        try withTemporarySQLiteDatabase(fileName: "fresh-v4.db") { temporary in
            let result = try MigrationCoordinator.prepareDatabase(at: temporary.databaseURL)
    ...
    func meetingCRUDAndOverlapWarnings() throws {
        try withLearningSceneFixture(fileName: "meetings.db") { fixture in
    ...
    func meetingValidation() throws {
        try withLearningSceneFixture(fileName: "meeting-validation.db") { fixture in
    ...
    func examCRUDAndReviewTask() throws {
        try withLearningSceneFixture(fileName: "exams.db") { fixture in
    ...
    func examValidation() throws {
        try withLearningSceneFixture(fileName: "exam-validation.db") { fixture in
    ...
    func relativeReminderFollowsDeadline() throws {
        try withLearningSceneFixture(fileName: "relative-reminders.db") { fixture in
    ...
    func relativeReminderRequiresDueDate() throws {
        try withLearningSceneFixture(fileName: "no-due-date.db") { fixture in
    ...
    func rescheduleRelativeReminders() throws {
        try withLearningSceneFixture(fileName: "reschedule.db") { fixture in
